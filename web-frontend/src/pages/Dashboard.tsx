...
import { getCardTitle, getAvailableCards } from '../types';
...
export default function Dashboard() {
  ...
  const renderCardContent = useMemo(() => {
    if (!summary) return (_card: LayoutCard) => <div className="text-zinc-400 text-sm">{t('dashboard.loading')}</div>;

    return (card: LayoutCard) => {
      switch (card.type) {
        case 'stats':
          return <StatsCard overview={summary.overview} lang={lang} />;
        case 'chart-holdings': {
          const data = summary.assets.filter(a => a.holding_cost_basis > 0.01).map(a => ({ name: a.name, value: a.holding_cost_basis }));
          return <ChartCard title={t('dashboard.holding_cost')} data={data} initialChartType={card.chartType} onChartTypeChange={(t) => changeChartType(card.id, t)} lang={lang} />;
        }
        case 'chart-type': {
          const data = Object.entries(summary.type_distribution).filter(([, v]) => v > 0).map(([k, v]) => ({ name: getTypeLabel(k, lang), value: v }));
          return <ChartCard title={t('dashboard.type_distribution')} data={data} initialChartType={card.chartType} onChartTypeChange={(t) => changeChartType(card.id, t)} lang={lang} />;
        }
        case 'chart-currency': {
          const data = Object.entries(summary.currency_distribution).filter(([, v]) => v > 0).map(([k, v]) => ({ name: k, value: v }));
          return <ChartCard title={t('dashboard.currency_distribution')} data={data} initialChartType={card.chartType} onChartTypeChange={(t) => changeChartType(card.id, t)} lang={lang} />;
        }
        case 'chart-tags': {
        ...
        case 'recent-trades':
          return <RecentTradesCard transactions={transactions} lang={lang} />;
        case 'holdings':
          return <HoldingsCard assets={summary.assets} lang={lang} />;
        default:
          return <div className="text-zinc-400 text-sm">{t('dashboard.loading')}</div>;
      }
    };
  }, [summary, transactions, changeChartType, t, lang]);
    ...
    return (
      <div className="p-4 md:p-8">
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-xl p-4">
          <p className="text-sm text-red-700 dark:text-red-400">{t('common.failed')}: {error}</p>
          <button onClick={() => { setError(null); loadAllData(); }} className="mt-2 px-3 py-1 bg-red-600 text-white rounded text-xs">{t('dashboard.retry')}</button>
        </div>
      </div>
    );
  }

  if (!layout || !summary) return <div className="p-4 md:p-8 text-center text-zinc-400">{t('dashboard.loading')}</div>;
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="relative" ref={addMenuRef}>
          <button onClick={() => setShowAddMenu(!showAddMenu)} className="flex items-center gap-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm">
            <Plus size={16} /> {t('dashboard.add_card')}
          </button>
          {showAddMenu && (
          ...
          >
            {layout.cards.map(card => (
              <KanbanCard
                key={card.id}
                id={card.id}
                title={getCardTitle(card.type, lang)}
        ...
        <DragOverlay>
          {activeId ? (
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl p-4 opacity-80">
              {getCardTitle(layout.cards.find(c => c.id === activeId)?.type ?? 'stats', lang)}
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
