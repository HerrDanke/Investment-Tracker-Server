...
**上下文：**
- 这些模型与桌面版 Tauri 应用共享数据格式（JSON 兼容）
- `#[serde(alias = ...)]` 确保 JSON 反序列化兼容 camelCase 和 snake_case
- `Database._seq` 用于自增 ID

**验证：**
```bash
cargo check
```
预期：通过（models.rs 独立编译，不依赖其他模块）
